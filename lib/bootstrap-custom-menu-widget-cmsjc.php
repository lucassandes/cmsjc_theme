...
add_action( 'widgets_init', 'register_bootstrap_custom_menu_cmsjc');

function register_bootstrap_custom_menu_cmsjc() {
    register_widget( 'bootstrap_custom_menu_cmsjc');
}

class bootstrap_custom_menu_cmsjc extends WP_Widget {

    function bootstrap_custom_menu_cmsjc() {

        $widget_ops = array( 'classname' => 'bootstrapwidgetmenucmsjc', 'description' => __('Not A custom menu widget that uses the wp_bootstrap_navwalker', 'devdmbootstrap3'));

        $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'bootstrap-widget-menu-cmsjc');

        $this->WP_Widget( 'bootstrap-widget-menu-cmsjc', __('Boot Strap Menu 2', 'devdmbootstrap3'), $widget_ops, $control_ops);
    }
